...
export const STR = {
  he: {
    ...
    // the one-line privacy note in every footer, and the page behind it.
    // Every sentence here describes what the code does; "no cookies" would be
    // false (a seller's login is a cookie) so it does not say that.
    privacyNote: "בלי מעקב, בלי פרסומות. הרשימה שלכם נשמרת רק במכשיר שלכם.",
    ...
    privNot2: "הגופנים נטענים משרתי Google Fonts, כך ש-Google רואה את כתובת ה-IP שלכם כשהעמוד נטען — כמו בכל אתר שמשתמש בהם. מעבר לזה, רק שירות המייל ששולח את המיילים שלנו (קודי כניסה, ועדכונים על פריטים שחזרו) רואה את הכתובת שאליה הוא שולח.",
    privWhereH: "איפה זה נשמר",
    privWhere1: "התמונות ופרטי המכירות נשמרים ב-Supabase, בשרתים באיחוד האירופי. האתר עצמו מוגש דרך Vercel.",
    privContact: "שאלות? כתבו לכתובת שממנה הגיע קוד הכניסה שלכם.",
    privBack: "חזרה",
    ...
    withdrawErr: "לא הצלחנו לבטל את הבקשה. נסו שוב.",
    ...
    wantMarked: "רק מה שסימנתי",
    nothingFree: "כל מה שנשאר כבר נתפס.",
    photoOf: (n: number, total: number) => `תמונה ${n} מתוך ${total}`,
    zoomIn: "להגדיל את התמונה", zoomOut: "לסגור",
    prevPhoto: "התמונה הקודמת", nextPhoto: "התמונה הבאה",
    ...
    confirmRemoveReq: (n: string) =>
      `להסיר את הבקשה של ${n}? כל מה שהיא מחזיקה יחזור למכירה.`,
    ...
    deleteWithPhotos: "מחק גם את התמונות עצמן",
    itemAndPhotosDeleted: "הפריט והתמונות נמחקו.",
    ...
    linkSentSpam: "לא רואים אותו? חפשו בתיקיית הספאם.",
    resend: "לשלוח שוב",
    useAnotherMail: "לשלוח לאימייל אחר",
    noSuchAccount: "אין מכירה עם האימייל הזה. אפשר לפתוח אחת חדשה.",
    waitToResend: (s: number) => `יש לחכות עוד ${s} שניות לפני שניתן לנסות לשלוח שוב`,
  ...
  en: {
    ...
    privacyNote: "No tracking, no ads. Your list is kept on your own device only.",
    privacyLink: "Privacy",
    privacyTitle: "What happens to your information",
    privBuyersH: "If you're buying",
    privBuyers1: "There is no account and no signup. Your wish list is kept in your browser's local storage, on your device only — not with us. You can clear it any time from your browser settings.",
    privBuyers2: "When you send a list, the name and phone number you typed are stored with us, so the seller knows who asked for what and can reach you. That is all that is stored, and only the seller of that sale sees it. Cancelling the request deletes it.",
    privBuyers3: "If you asked to hear when an item is back, your email address is kept only until that email is sent, then deleted. If the item sells instead, it is deleted and nothing is sent. The seller never sees it. The email goes out through Resend, an email delivery service.",
    privSellersH: "If you're selling",
    privSellers1: "To open a sale we keep an email, a name and a WhatsApp number. The email is used only to send a sign-in code. The phone number appears nowhere public — a buyer gets it only after asking for something, in order to message you.",
    privSellers2: "Being signed in is kept in one cookie, whose only job is to remember that you are. It is the only cookie on the site.",
    privNotH: "What isn't here",
    privNot1: "No analytics, no pixels, no tracking, no ads. We do not know how many people visited a sale, and do not try to.",
    privNot2: "Fonts are loaded from Google Fonts, so Google sees your IP address when a page loads — as on any site that uses them. Beyond that, only the email service that sends our emails (sign-in codes, and notes that an item is back) sees the address it sends to.",
    privWhereH: "Where it lives",
    privWhere1: "Photos and sale details are stored in Supabase, on servers in the European Union. The site itself is served by Vercel.",
    privContact: "Questions? Write to the address your sign-in code came from.",
    privBack: "Back",
    ...
    myList: "My list",
    pendingNote: "You've sent a request. You can cancel it until the seller settles it with you.",
    withdraw: "Cancel my request",
    withdrawConfirm: "Cancel the request? The items go back on sale and your list stays as it is.",
    withdrawn: "Request cancelled. Your list was kept.",
    withdrawGone: "That request is gone already — the seller may have dealt with it.",
    withdrawErr: "We couldn't cancel the request. Try again.",
    ...
    waitAsk: "Tell me if it's back",
    waitEmail: "Your email",
    waitSend: "Tell me",
    waitDone: "We'll email you the moment it's back on sale.",
    waitBadEmail: "That doesn't look like an email address.",
    waitBack: "It's just come back on sale!",
    waitErr: "We couldn't save that. Try again.",
    ...
    statSold: "sold",
    statEarned: "taken in",
    requestsH: "Requests that came in",
    ...
    removeReq: "Remove",
    confirmRemoveReq: (n: string) =>
      `Remove ${n}'s request? Everything it holds goes back on sale.`,
    reqRemoved: "Request removed, and the items are back on sale.",
    reqRemoveErr: "We couldn't remove the request.",
    waReply: (buyer: string, seller: string) => `Hi ${buyer}! ${seller} here about the garage sale. When suits you for pickup?`,
    copied: "Link copied",
    flyer: "Get a link and an ad for the sale",
    flyerTitle: "Sale flyer",
    flyerMaking: "Making the flyer…",
    ...
    deleteWithPhotos: "Delete the photos themselves too",
    itemAndPhotosDeleted: "Item and photos deleted.",
    ...
    priceForAll: "the whole lot",
    priceForHint: "Say more in the description; the final price gets settled on WhatsApp anyway.",
    ...
    errSize: "Fill in the measurements",
    sizeW: "width", sizeL: "height", sizeD: "depth",
    sizeAsk: "Better to give all three measurements. Continue anyway?",
    sizeAskYes: "Yes, continue",
    sizeAskNo: "No, fix them",
    ...
    linkSentSpam: "Not in your inbox? Look in the spam folder.",
    resend: "Send it again",
    useAnotherMail: "Send to another email",
    noSuchAccount: "No sale with that email yet. You can start one.",
    waitToResend: (s: number) => `Wait another ${s} seconds before sending again`,
