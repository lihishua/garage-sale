...
3. **Authentication → Providers → Email**: ודאי שהוא פעיל.
   בשלב הפיתוח כדאי לכבות *Confirm email* כדי לא לחכות למיילים.
4. **Project Settings → API**: העתיקי את `Project URL` ואת `anon public`.
...
### 3. מסלול בדיקה
1. "לפתוח מכירת חצר משלי" → הרשמה עם אימייל, סיסמה, שם, טלפון וכתובת עמוד (למשל `dana`).
